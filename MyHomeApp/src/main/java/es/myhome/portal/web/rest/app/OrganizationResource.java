...
public class OrganizationResource {
    ...
    public ResponseEntity<OrganizationDTO> updateOrganization(@Valid @RequestBody OrganizationDTO organizationDTO) {
        ...
        return ResponseUtil.wrapOrNotFound(updatedOrganization, HeaderUtil.createAlert(applicationName, "userManagement.updated", organizationDTO.getName() ) );
    }
    ...
    /**
     * {@code GET /admin/organizations/:login} : get the "login" user.
     *
    ...
    @PreAuthorize("hasAuthority(\"" + AuthoritiesConstants.ADMIN + "\")")
    public ResponseEntity<OrganizationDTO> getOrganization(@PathVariable @Pattern(regexp = Constants.ENTITIES_ID_REGEX) String idOrganization) {
        log.debug("REST request to get Organization : {}", idOrganization);
        return null;//ResponseUtil.wrapOrNotFound(userService.getUserWithAuthoritiesByLogin(login).map(AdminUserDTO::new));
    }
    ...
    @PreAuthorize("hasAuthority(\"" + AuthoritiesConstants.ADMIN + "\")")
    public ResponseEntity<Void> deleteOrganization(@PathVariable @Pattern(regexp = Constants.ENTITIES_ID_REGEX) String idOrganizationStr) throws Exception {
        log.debug("REST request to delete Organization: {}", idOrganizationStr);
        Long idOrganization = null;
		try {
			idOrganizationStr = idOrganizationStr.replaceAll("\\D+", "");
			idOrganization = Long.parseLong( idOrganizationStr );
        ...
        return ResponseEntity.noContent().headers(HeaderUtil.createAlert(applicationName, "organizationService.deleted", idOrganization.toString())).build();
    }
